...
T=zeros(D);
j=0;
%Esto seria Lanczos para encontrar el mínimo y el máximo autovalor en el
%tiempo minimo.

while max(eig(A))~=max(eig(T)) && min(eig(A))~=min(eig(T))
    %j
   if j==0
    r0=rand(D,1);
    b0=norm(r0);
...
%T=q'*A*q Produce errores de redondeo, por lo que no es viable
 ...
 %Debido a que se producen errores en el
